...
def build_experiment(config) -> Experiment:
    k_min, k_max = parse_clustering_limits(config)
    distance = parse_distance(config)
    labeling = km.Labeling(distance)
    initialization = parse_initialization(config, distance)
    number_of_iterations = parse_number_of_iterations(config)
    kmeans = km.KMeans(labeling=labeling, initialize=initialization,
                       number_of_iterations=number_of_iterations)

    gap_pool, grouping_pool = spawn_pool(config)

    gap_trials = parse_gap_trials(config)
